...
**URL:** https://exult.sourceforge.io/studio.html
...
Of course the terms explained in ReadMe are also used in this Documentation but we will list them along with the terms that additionally need to be explained for the usage of Exult Studio.
...
flex files:
also known as flexes, these are used by the original to store the data files. Also when you choose to not use compressed save games (see save_compression_level in ReadMe) our save games are also "flexed".
gumps:
...
You need Exult and you need to install it properly (see ReadMe). Also make sure that you are using the latest snapshot of Exult.
Exult Studio only works with the latest snapshot, not the latest release.
For U*ix platforms you need to build Exult Studio from the source available from our source snapshot at https://exult.info/download.html.
...
Enable the Exult cheat mode (see ReadMe).
...
While it is not explicitly needed it is useful to have a working copy of either BG or SI properly installed onto your computer.
Next, you need to have the latest Exult snapshot installed properly onto your computer and working correctly. If you are currently playing either BG or SI on your machine this is probably done right. To make sure you have the correct version, ensure that it says X.XX GIT (X.XX stands for some version number) during Exult's start-up screen (for example see https://exult.info/images/screenshots/mainmenu.png).
...
The patch folder needs to be in folder which you are allowed to write, so Exult Studio can save any changes to existing games. See 2.4. What you can do at the moment on how to change exult.cfg accordingly.
In order for Exult Studio to work properly, you need to have Exult running in windowed mode. If you can see your desktop while playing, then you're doing it right.
Make sure you have Cheats enabled. This can be enabled in the Gameplay options of the Game see (see ReadMe).
While running Exult in windowed mode, hit 'ctrl+alt+m' to enter map-editing mode. Exult Studio should open up if you did everything correctly. In map-editing mode the world is frozen, but you are still able to move your characters around in order to place objects. You're now ready to start your world editing. You should be seeing something like this:
...
<default_game>	** default game to edit (games must be specified in<game> part of exult.cfg)
gamename
</default_game>
<image_editor>	** command that calls the image editor.
gimp-remote -n
</image_editor>
<background_color>	** background color for the shape-chooser. Use the preferencedialog of Exult Studio for this.
0
</background_color>
</estudio>
...
For an example of how Usecode may look like download our SF Island mod for BG from our download page.
...
These can be built from the Exult source or you can download the Windows binary snapshot. The source and binary snapshot can be found at https://exult.info/download.html.
The tools are:
...
See the manual.
The GIMP Plug-in:
can be used for editing shapes in the GIMP. Handles multiframe Shape files. See http://www.gimp.org for information on the GIMP.
Photoshop Plug-in:
can be used for editing shapes in Photoshop. Can't handle multiframe Shape files. And also can't handle indexed images with Alpha correctly.

 
9.5. Porting Exult Studio to other platforms


Currently Exult Studio runs on U*ix and Windows platforms. First it only ran on U*ix but with some effort by Colourless, Kirben and Mark Siewart it was successfully ported to Windows.
...
Recently we switched from using X drag'n'drop to SDL2's DropFile. For Windows Exult we are still using our own code.
As an alternative to dragging objects, you can now select a shape, then insert it into the game window by left-clicking while holding the Shift key down.
...
This confused me too, and I originally planned on doing away with it. But do a little arithmetic: Ultima VII's world has 3072x3072 tiles; that's over 9 million that need to be set. My guess is that they came up with the idea behind 'u7chunks' to save artists time and to give the map a consistent look.

The basic idea is that each 'chunk' of flat terrain, 16x16 tiles, is stored in 'u7chunks', and is indexed by its position in this file (0, 1, 2, ...). The 'u7map' file specifies, for each 'chunk' in the Ultima VII map, which of the terrains is to be used. Since the entire map has 192x192 chunks, or a total of 36,864, and since 'u7chunks' has 3072 entries, each terrain is shared an average of over 10 times. When you go into map-editing mode in Exult, the numbers displayed are the indices from 'u7chunks'.

So here are the three sorts of scenery that you can see and edit:

The 'terrain' (flat landscape, rocks, shrubs, and first-story buildings) specified in 'u7chunks' and 'u7map'.
Other non-moveable objects (second-floor structures, trees) specified in the 'ifixnn' files.
...
For a more detailed explanation, see Gary Thompson's paper, u7edit.txt, in our 'docs' folder.
...
Crowley from our forum made this chart to easily reference the u7iregnn and u7ifixnn files in both BG and SI. Thanks a lot!
...
when you load games or mods with more than one map, Exult Studio has problems creating and saving the extra maps. This may no longer be an issue.
10. Credits
 
10.1. Thanks

Colourless, Kirben and Mark Siewart invested a lot of effort to port Exult Studio to Windows.
11. Changes to the Documentation
 
11.1. - 20 February 2021

Changed links to our homepage to exult.info, SDL to SDL2, Mac OS X to macOS.
Updated all the menus and editors descriptions.
Many little changes to wordings and indentation.

 
11.2. - 13 March 2016

2.3. Step-by-step instructions to run Exult Studio GIT vs SVN.

 
11.3. - 21 November 2013

9.8. ifix and ireg reference chart finally added Crowley's ifix/ireg chart.

 
11.4. - 19 February 2011

2.6. Command line options added -c -m, added space between -x -g, even though it works without space as well.

 
11.5. - 23 January 2011

11. Changes to the Documentation moved to the bottom.
2.6. Command line options added portable option.

 
11.6. - 18 December 2010

5.3. Custom Usecode added intriniscs docs from TFL and fixed the link to Pertex' site.
9.10. Known Bugs with the Linux port the patch folder NEEDS to be writeable.

 
11.7. - 11 March 2010

2.1. Installation added Vista/W7 to the Windows list, Windows 95 is not supported.
2.4. What you can do at the moment the patch folder NEEDS to be writeable.
1.1. Windows 95 is not supported.

 
11.8. - 01 March 2005

4.3. Edit Create Barg added.
4.6. Map added this menu option.
6.2. The different types of eggs intermap egg added.
6.14. Intermap eggs explains how to use this egg type.
6.15. Multi-map support explained.
9.1. Creating a boat, flying carpet, or horse drawn carriage added.
9.2. Importing and creating your own custom shapes added.
9.3. How to create a custom font? explained.

 
11.9. - 23 May 2004

3.4. Palettes files palette files are compatible with the Gimp.
4.3. Edit note about map-editing operations.
6.4. Properties of eggs some more infos.
6.6. Schedules of Monsters and Creatures more on the patrol schedule.
7.5. NPC Flags removed some unused flags.

 
11.10. - 06 February 2004

1.3. Terms we use in regard to Exult and Ultima VII some corrections by Willem.
2.3. Step-by-step instructions to run Exult Studio some corrections.
2.2. Running Exult Studio some corrections.
3.3. Map files More right click options.
7. NPC editing finally took on that task.
...
Last modified: 20 February 2021 (GMT)
