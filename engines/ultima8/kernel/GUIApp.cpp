...
void GUIApp::startup()
{
	...
	SDL_HideCursor();
	SDL2_GetMouseState(&mouseX, &mouseY);
...
void GUIApp::handleEvent(const SDL_Event& event)
{
	...
	switch (event.type) {
		case SDL_KEYDOWN:
			key = HID_translateSDLKey(event.key.key);
		...
		if (gump) {
			switch (event.type) {
				case SDL_KEYDOWN:
					...
					if (event.key.key == SDLK_v && event.key.mod & KMOD_CTRL)
					{
					...
					if (event.key.key == SDLK_v && event.key.mod & KMOD_META)
					{
					...
					if (0 /* SDL3: use SDL_EVENT_TEXT_INPUT instead */ >= ' ' &&
						0 /* SDL3: use SDL_EVENT_TEXT_INPUT instead */ <= 255 &&
						!(0 /* SDL3: use SDL_EVENT_TEXT_INPUT instead */ >= 0x7F && // control chars
						  0 /* SDL3: use SDL_EVENT_TEXT_INPUT instead */ <= 0x9F))
					{
						gump->OnTextInput(0 /* SDL3: use SDL_EVENT_TEXT_INPUT instead */);
					}

					gump->OnKeyDown(event.key.key, event.key.mod);
					return;

				case SDL_KEYUP:
					gump->OnKeyUp(event.key.key);
					return;
