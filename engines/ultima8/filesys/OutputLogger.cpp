...
// SDL3 thread function wrapper
static int SDLCALL OutputLoggerThreadFunc(void *data)
{
	OutputLogger *logger = static_cast<OutputLogger*>(data);
	return logger->ThreadMain();
}
	...
	fdPipeRead(-1), pThread(NULL)
{
	...
	// Create the thread (SDL3 API)
	pThread = SDL_CreateThread(OutputLoggerThreadFunc, "OutputLogger", this);
}
